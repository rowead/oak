...
class Window extends EventEmitter2 {
  constructor (core, opts, callback = function () {}) {
    ...
    let args = {
      ...
      transparent: _this.opts.transparent
    }
    ...
    this.instance = new BrowserWindow(args)
  ...
  location (newUrl) {
    ...
    this.emit('location', { url: oldUrl, session: oldSession }, { url: newUrl, session: newSession })
    return this
  }
  ...
  reload (cache = true) {
    ...
    // start new session
    let newSession = this.newSession()
    cache ? this.instance.webContents.reload() : this.instance.webContents.reloadIgnoringCache()
